 ...
 * @license CeCILL-B V1
 */
class DeletedNetworkManager implements LoggableManagerInterface, PaginatorInterface
{
    /**
    ...
    public function getOtherLogEntriesQuery($log)
    {
      ...
      $qb = $this->em->createQuery('SELECT log FROM Gedmo\Loggable\Entity\LogEntry log WHERE log.objectId = ?1 AND log.objectClass = \'App\Entity\Network\' ORDER BY log.version DESC')
    ...
    public function getQueryBuilder()
    {
        ...
        $qb->where(self::ALIAS.'.objectClass = \'App\Entity\Network\'')
            ->andWhere(self::ALIAS.'.action = \'create\'')
            ->andWhere(self::ALIAS.'.objectId IN (SELECT log.objectId FROM Gedmo\Loggable\Entity\LogEntry log WHERE log.action = \'remove\' AND log.objectClass = \'App\Entity\Network\')');
