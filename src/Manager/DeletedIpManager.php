...
class DeletedIpManager implements LoggableManagerInterface, PaginatorInterface
{
    ...
    public function getOtherLogEntriesQuery($log)
    {
      ...
      $qb = $this->em->createQuery('SELECT log FROM Gedmo\Loggable\Entity\LogEntry log WHERE log.objectId = ?1 AND log.objectClass = \'App\Entity\Ip\' ORDER BY log.version DESC')
